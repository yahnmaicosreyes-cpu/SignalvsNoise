...
The MVP detection engine is complete and works end-to-end. You can paste a text dump of your week, and the engine returns a printable weekly timesheet (Monday–Sunday) with every scheduling problem flagged inline.
...
# Render a printable Monday-through-Sunday timesheet:
rendered = render_timesheet(result.events, issues)
print(rendered)

# Export to a .txt file (date-stamped, refuses to overwrite):
from src.engine.timesheet_exporter import export_timesheet
path = export_timesheet(rendered, directory=".")
print(f"Saved to: {path}")
```

## Timesheet Format

`render_timesheet(events, issues)` produces a plain-text weekly view:

- Title line: "Weekly Timesheet"
- All seven days shown Mon→Sun, even empty days (marked "Open Availability")
- Banner-style day headers (40 equals signs)
- Each event line shows: `HH:MM - HH:MM   title   [QUADRANT]`
- Issues appear as indented sub-lines beneath each related event, prefixed with `!`
- Title column auto-aligns to the longest title in the week
- 24-hour military time format throughout
...
Expected: `GRAND TOTAL: 322/322 tests passed`.
...
- [x] Mon-Sun timesheet generation (formatted printable view)
- [x] Print-friendly export (.txt file with safety checks)
